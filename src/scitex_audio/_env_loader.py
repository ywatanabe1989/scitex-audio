#!/usr/bin/env python3
"""Environment variable loader for SCITEX_AUDIO_ENV_SRC.
...
This enables the MCP server to pick up relay, backend, and API key settings
...
def load_scitex_audio_env() -> int:
    """Load environment variables from SCITEX_AUDIO_ENV_SRC if set.

    Call this early in MCP server startup to pick up relay, backend,
    and API key settings from shell profile .src files.
